...
const HFT_RULES = {
    ...
    generateRecommendation(state) {
        ...
        // NEW RULE: OS = 10% of total cores (minimum 2)
        let osNeeded = Math.max(2, Math.ceil(totalCores * 0.1));
        // If we are forced to use isolated cores for OS, cap at available cores
        ...
        // NEW RULE: IRQ = 1 per 4 gateways (minimum 2)
        const calculatedGw = calcNeeded(currentRoles['gateway']);
        const neededIrq = Math.max(2, Math.ceil(calculatedGw / 4));
        // Add 20% safety margin to Gateway count
        const neededGw = Math.ceil(calculatedGw * 1.2);
