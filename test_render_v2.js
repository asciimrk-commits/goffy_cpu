...
// Check for hierarchy and ABSENCE of label
if (html.includes('class="socket"') &&
    html.includes('class="numa') &&
    html.includes('class="l3"') &&
    html.includes('class="phy-core"') &&
    !html.includes('Core P#0')) { // Make sure label is GONE
    console.log('Render Structure Verified (Labels Removed)');
} else {
    console.error('Render Structure Failed');
    if (html.includes('Core P#0')) console.error('Error: "Core P#0" label still present');
    console.log(html);
    process.exit(1);
}
